                    ...
                    <tr v-for="utente in $page.props.utenti" :key="utente.id">
                        ...
                        {{ utente.username }}
                            <dl class="font-normal lg:hidden">                                            
                                <dd class="mt-1 truncate text-gray-700">{{ utente.medico ? utente.medico.ragione_sociale : '' }}</dd>                                           
                                <dd class="mt-1 truncate text-gray-500 sm:hidden">{{ utente.attivo }}</dd>
                            </dl>
                        </td>
                        <td class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">{{ utente.medico ? utente.medico.ragione_sociale : '' }}</td>
                        <td class="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell">{{ utente.attivo ? 'SI' : 'NO' }}</td>
                        <td class="px-3 py-4 text-sm text-gray-500">{{  utente.role?.name }}</td>
                        <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <button @click="apriModalModifica(utente.id)" class="text-indigo-600 hover:text-indigo-900">
                                Modifica
                            </button>
                        </td>
                    </tr>
