                                ...
                                <tr v-for="utente in $page.props.utenti" :key="utente.id">
                                    ...
                                    {{ utente.sphere_user.username }}
                                        <dl class="font-normal lg:hidden">                                            
                                            <dd class="mt-1 truncate text-gray-700">{{ utente.sphere_user.medico ? utente.sphere_user.medico.nome : '' }}</dd>                                           
                                            <dd class="mt-1 truncate text-gray-500 sm:hidden">{{ utente.sphere_user.attivo }}</dd>
                                        </dl>
                                    </td>
                                    <td class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">{{ utente.sphere_user.medico ? utente.sphere_user.medico.nome : '' }}</td>
                                    <td class="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell">{{ utente.sphere_user.attivo }}</td>
