<template>
    <TransitionRoot as="template" :show="open">
        <Dialog as="div" class="relative z-50">
                        ...
                        <DialogPanel
                            ...
                            <div v-else>
                                <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900 text-center">
                                    {{ prenotazione.id ? 'Modifica prenotazione medsport' : 'Nuova prenotazione medsport' }}
                                </DialogTitle>
                                <div class="flex flex-col gap-3">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        ...
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <p v-if="!orarioInOrarioMedico && $page.props.settings.avviso_presenza_orario_medico.value"
                                            ...
                                            <div class="flex items-center gap-3">
                                                <div class="flex flex-col gap-1">
                                                    <DxButton width="100%" text="Leggi Tessera Sanitaria" type="normal"
                                                        icon="find" styling-mode="contained" :disabled="false"
                                                        @click="letturaTessera" />
                                                    <DxButton id="saveButton" width="100%" text="Nuovo paziente"
                                                        icon="plus" type="normal" styling-mode="contained"
                                                        :disabled="false" @click="apriModalNuovoPaziente()" />
                                                </div>

                                                <DxButton id="saveButton" width="100%" height="100%"
                                                    text="Apri paziente" icon="activefolder" type="normal"
                                                    styling-mode="contained"
                                                    :disabled="!prenotazione.visita.paziente_id"
                                                    @click="apriModalModificaPaziente" />
                                            </div>
                                            <DxSelectBox ref="selectboxPaziente"
                                                v-model:value="prenotazione.visita.paziente_id"
                                                :data-source="pazientiDataSource" value-expr="id"
                                                display-expr="ragione_sociale" :search-enabled="true"
                                                :search-timeout="500" label="Paziente" label-mode="static"
                                                placeholder="Cognome e Nome" no-data-text="Nessuna anagrafica trovata"
                                                :disabled="disabledElement" :drop-down-options="{ maxHeight: '200px' }"
                                                :item-template="pazienteItemTemplate"
                                                :on-selection-changed="selezionaPaziente"
                                                :is-valid="!errorFor('visita.paziente_id')" :open-on-field-click="false"
                                                :min-search-length="2" />

                                            <div v-if="avviso_paziente">
                                                AVVISO PAZIENTE
                                            </div>
                                            ...
                                            <DxSelectBox v-model:value="prenotazione.visita.sport_id"
                                                label="Sport praticato" :data-source="elenco_sport" value-expr="id"
                                                display-expr="nome" :search-enabled="true"
                                                :item-template="sportItemTemplate"
                                                :is-valid="!errorFor('visita.sport_id')" :disabled="disabledElement"
                                            ...
                                            <DxSelectBox v-model:value="prenotazione.societa_id"
                                                :data-source="societaDataSource" value-expr="id"
                                                display-expr="ragione_sociale" :search-enabled="true"
                                                :search-timeout="500" label="Società sportiva" label-mode="static"
                                                no-data-text="Nessuna società trovata" :disabled="disabledElement"
                                                :drop-down-options="{ maxHeight: '200px' }"
                                                :on-selection-changed="selezionaSocieta"
                                                :is-valid="!errorFor('societa_id')" :open-on-field-click="false"
                                                :min-search-length="2" />
                                        </div>
                                        <div class="flex flex-col gap-1">
                                            <div class="relative">
                                            ...
                                            <div v-if="!prenotazione.visita.paziente"
                                            ...
                                            <div v-else>
                                                <div class="flex justify-between  pt-1">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente.nome }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Cognome: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente.cognome }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">ragsoc: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente.ragione_sociale }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Sesso: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente.sesso }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Data di nascita: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ dateFormat(prenotazione.visita.paziente.data_nascita) }}
                                                ...
                                                <div class="flex justify-between  pt-1">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-gray-900 uppercase">{{ prenotazione.visita.paziente.localita_nascita?.nome }}
                                                        (
                                                ...
                                                <div class="flex justify-between  pt-1">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-gray-900 uppercase">{{ prenotazione.visita.paziente?.codice_fiscale }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Residenza: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 uppercase">{{ prenotazione.visita.paziente.localita_residenza?.nome }}
                                                        (
                                                ...
                                                <div class="flex justify-between  pt-1">
                                                    ...
                                                    <span
                                                        class="text-sm text-gray-900 uppercase text-right">{{ prenotazione.visita.paziente?.indirizzo }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Cellulare: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente?.telefono }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Fisso: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente?.telefono_fisso }}</span>
                                                </div>
                                                <div class="flex justify-between  pt-1">
                                                    <span class="text-sm text-gray-500">Indirizzo Email: </span>
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.visita.paziente?.email }}</span>
                                                </div>
                                            </div>
                                            <div class="relative">
                                                <div class="absolute inset-0 flex items-center" aria-hidden="true">
                                            ...
                                            <div v-if="!prenotazione.societa_sportiva"
                                                class="flex justify-center items-center h-full">
                                            ...
                                            <div v-else>
                                                <div class="flex justify-between  pt-1">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium text-gray-900 capitalize">{{ prenotazione.societa_sportiva.ragione_sociale }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 pt-4">
                                        <DxTextArea v-model:value="prenotazione.note" placeholder="Note prenotazione"
                                            :spellcheck="false" />
                                    </div>
                                </div>

                                <div class="mt-5 sm:mt-6 flex flex-col sm:flex-row sm:justify-around gap-4">
                                    <DxButton width="100%" text="Chiudi" type="danger" icon="close"
                                        styling-mode="contained" :disabled="saving" @click="$emit('close')" />
                                    <DxButton id="saveButton" width="100%"
                                        ...
                                        :disabled="disabledElement || saving" @click="salvaPrenotazione()" />
                                </div>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
            ...
            <ModalPaziente v-if="modalPazienteId" :paziente-id="modalPazienteId" :dati-tessera="datiTesseraSanitaria"
                @close="modalPazienteId = null" @store="cambiaPaziente" @update="cambiaPaziente" />
        </Dialog>
    </TransitionRoot>
</template>
