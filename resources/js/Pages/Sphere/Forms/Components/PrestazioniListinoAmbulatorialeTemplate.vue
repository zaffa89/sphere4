        ...
        >
            ...
            <DxSummary :recalculate-while-editing="true">
                <DxTotalItem
                    ...
                    display-format="Prezzo totale prestazioni associate € {0}"
                    show-in-column="prezzo"
                />
            </DxSummary>
        </DxDataGrid>
    </div>
...
<script setup>
    ...
    const prestazioniAssociate = { 
        store: new CustomStore({
                key: 'id',
                load: () => axios.get(`api/sphere/ambulatoriale/prestazioni-listino/${props.templateData.data.id}`).then(response => response.data).catch(err => { throw new Error('Errore caricamento'); }),
                insert: (key) => axios.post(`api/sphere/ambulatoriale/prestazioni-listino/add/${props.templateData.data.id}/${key}`).then(response => response.data).catch(err => { throw new Error('Errore caricamento'); }),        
                remove: (key) => axios.delete(`api/sphere/ambulatoriale/prestazioni-listino/remove/${props.templateData.data.id}/${key}`),
                errorHandler: (error) => {
                    console.log(error.message);
                },
                onPush: (changes) => {            
                    axios.post(`api/sphere/ambulatoriale/prestazioni-listino/add/${props.templateData.data.id}/${changes[0].data.id}`).then(response => response.data).catch(err => { throw new Error('Errore caricamento'); })          
                }        
            }),
        reshapeOnPush: true
    }
