<template>
    <div>
      <DxDataGrid
        id="griglia"
        ref="griglia"
        :data-source="$page.props.pazienti"
        key-expr="id"
      ...
      >
        ...
        />
        <DxColumn
          data-field="localita_residenza.nome"
          caption="Località di residenza"
        />
        <DxColumn
          data-field="localita_residenza.sigla_provincia"
          caption="Provincia di residenza"
        />
        <DxColumn
          data-field="indirizzo"
          caption="Indirizzo"
        />
        <DxColumn
          data-field="civico"
        ...
        />
        <DxColumn
          data-field="localita_nascita.nome"
          caption="Località di nascita"
        />
        <DxColumn
          data-field="localita_nascita.sigla_provincia"
...
import AppLayout from '../../Layouts/AppLayout.vue';
  ...
  export default {
      ...
      async created() {
          locale('it-IT');
          //await axios.get('paziente').then(response => { this.pazienti = response.data; });
      },
      methods: {
          timezoneFixer(data) {
          ...
          deletePaziente(e) {
              e.cancel = axios.delete(`paziente/${e.data.id}`)
                  .then(response => {
          ...
          async ricercaPaziente() {
              this.fetching = true;
              await axios.post('ricerca-paziente', { ricerca: this.ricerca }).then(response => {
